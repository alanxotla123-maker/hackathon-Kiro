...
export default function GoogleCallback({ setAuthScreen, showNotification }: GoogleCallbackProps) {
    ...
    const processGoogleAuth = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || errorData.message || 'Error al autenticar con Google')
        }
        ...
        localStorage.setItem('profileImageUrl', data.user.profileImage || '')
        ...
        setTimeout(() => {
          ...
          showNotification(`Autenticado vía Google como ${data.user.name}`)
        }, 500)

      } catch (error: any) {
        console.error('Error en Google OAuth callback:', error)
        setStatus('error')
        setErrorMessage(error.message || 'Ocurrió un error inesperado durante la autenticación con Google.')
        ...
        {status === 'loading' && (
          <>
            <div style={{ marginBottom: '20px' }}>
              <svg viewBox="0 0 48 48" width="48" height="48" style={{ animation: 'pulse 2s infinite' }}>
            ...
            <h2 className="auth-app-title" style={{ fontSize: '24px', marginBottom: '10px' }}>Autenticando con Google</h2>
